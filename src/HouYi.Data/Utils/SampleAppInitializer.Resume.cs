...
{
    public static void SeedResumeData(HouYiDbContext db)
    {
        if (!db.Resumes.Any())
        {
            var cities = db.LeveledReferenceData
                .Where(data => data.Category == Constants.Categories.Area && data.Level == 3 && data.IsDeleted == false).ToList();
    ...
    private static List<Resume> GenerateResumes(int count, List<LeveledReferenceData> cities)
    {
